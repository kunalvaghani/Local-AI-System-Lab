...
| Windows setup/launcher | Backend-first Stage 25 launcher with health/mode checks | Stage 27 dependency-tree verification/`npm ci`, optional Ollama → backend → frontend order, tool/release contract checks, signature-verified stale project-service replacement, and unrelated-process protection | `setup_and_run.bat`, `local_stack_process.ps1`, `apps/web/public/local-ai-release.json`, `test_setup_launcher.py` |
